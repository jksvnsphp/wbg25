            ...
            <ul class="pagination custom-pagination">
                ...
                @else
                    <li class="page-item"><a class="page-link"
                            href="{{ $paginator->previousPageUrl() }}&pageItem={{ request('pageItem', 10) }}">&lsaquo;</a>
                    </li>
                @endif
                ...
                @if ($paginator->hasMorePages())
                    <li class="page-item"><a class="page-link"
                            href="{{ $paginator->nextPageUrl() }}&pageItem={{ request('pageItem', 10) }}">&rsaquo;</a>
                    </li>
                @else
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @php $url .= '&pageItem=' . request('pageItem', 10); @endphp
                    ...
                    <div class="d-flex flex-column">
                        <select name="pageItem" id="perPage" class="select-per-page"
                            ...
                            @foreach ([10, 25, 50, 100] as $size)
                                <option value="{{ $size }}"
                                    {{ request('pageItem', 10) == $size ? 'selected' : '' }}>{{ $size }}
                                </option>
                            @endforeach
                        </select>
