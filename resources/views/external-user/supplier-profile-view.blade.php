...
@section('meta_data')
<title>{{ optional($seller->profile_meta)->title ?? 'WBG24' }}</title>
<meta name="description" content="{{ optional($seller->profile_meta)->description ?? 'No Description' }}">
@php
$keywords = [optional($seller->profile_meta)->keywords ?? ''];

if ($seller->company) {
for ($i = 1; $i <= 10; $i++) {
    $value=$seller->company['key' . $i] ?? null;
    if (is_string($value)) {
    $value = trim($value);
    }
    if (!empty($value)) {
    ...
    $keywords = array_values(array_unique(array_filter($keywords, function ($v) {
    return is_string($v) ? trim($v) !== '' : !empty($v);
    })));
    $keywordsString = implode(', ', $keywords);
    @endphp
    <meta name="keywords" content="{{ $keywordsString }}">
    <meta name="author" content="{{ optional($seller->company)->name ?? '' }}">
    @endsection
                ...
                <div class="card-body">
                    <div class="row justify-content-center">
                        ...
                        <div class="col-sm-3">
                            ...
                            @if (isset($spotlights) && isset($spotlights->company))
                            <div class="card shadow pt-2 rounded-0">
                                <div class="card-img d-flex justify-content-center">
                                    <img src="@if (isset($spotlights->company->spotlight_banner) && $spotlights->company->spotlight_banner != '') {{ asset('uploads/profile/' . $spotlights->company->profile_banner) }} @elseif (isset($spotlights->company->spotlight_preview1) && $spotlights->company->spotlight_preview1 != '') {{ asset('uploads/profile/' . $spotlights->company->spotlight_preview1) }} @else https://placehold.co/400x400 @endif"
                                        alt="profile-image" style="height: 8rem;width: 100%;" class="mx-auto" />
                                </div>
                                <div class="card-body text-center">
                                    <a href="{{ url('/' . $spotlights->ref_no) }}"
                                        class="text-decoration-none text-primary">View More <i
                                            class="fa fa-arrow-right"></i>
                                    </a>
                                </div>
                                ...
                                <div class="col-8">
                                    <div class="progress rounded-0" style="height: 9px; margin: 8px 0">
                                        <div class="progress-bar bg-success" role="progressbar" aria-valuenow="5"
                                            ...
                                            data-width="{{ $ratingData['fiveStarPercent'] ?? 0 }}" style="width: 0%;">
                                ...
                                <div class="col-8">
                                    <div class="progress rounded-0" style="height: 9px; margin: 8px 0">
                                        <div class="progress-bar bg-primary" role="progressbar" aria-valuenow="5"
                                            aria-valuemin="0" aria-valuemax="5"
                                            data-width="{{ $ratingData['fourStarPercent'] ?? 0 }}" style="width: 0%;">
                                            <span class="sr-only"></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-8">
                                    <div class="progress rounded-0" style="height: 9px; margin: 8px 0">
                                        <div class="progress-bar bg-info" role="progressbar" aria-valuenow="5"
                                            aria-valuemin="0" aria-valuemax="5"
                                            data-width="{{ $ratingData['threeStarPercent'] ?? 0 }}" style="width: 0%;">
                                            <span class="sr-only"></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-8">
                                    <div class="progress rounded-0" style="height: 9px; margin: 8px 0">
                                        <div class="progress-bar bg-warning" role="progressbar" aria-valuenow="5"
                                            aria-valuemin="0" aria-valuemax="5"
                                            data-width="{{ $ratingData['twoStarPercent'] ?? 0 }}" style="width: 0%;">
                                            <span class="sr-only"></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-8">
                                    <div class="progress rounded-0" style="height: 9px; margin: 8px 0">
                                        <div class="progress-bar bg-danger" role="progressbar" aria-valuenow="5"
                                            aria-valuemin="0" aria-valuemax="5"
                                            data-width="{{ $ratingData['oneStarPercent'] ?? 0 }}" style="width: 0%;">
                                            <span class="sr-only"></span>
                                        </div>
                                    </div>
                                </div>
