            ...
            <div class="col-md-12 mb-4">
                ...
                @if ($groupedCarts)
                    ...
                    @foreach ($groupedCarts as $cart)
                        ...
                        @php
                            if(isset($cart->quotation->id)){
                                $itemNumbers .= str_pad($cart->quotation->id, 9, '0', STR_PAD_LEFT).' ';
                            }
                            $totalPrice += $cart->offer_price;
                        @endphp
                    @endforeach
                @endif
                <form method="post" enctype="multipart/form-data" action="{{ route('order.now.quotation') }}"
                    class="row mt-5">
                    @csrf
                        ...
                        <div class="card rounded-0  border-1 mt-3">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <strong class="fw-bold">Article ({{ count($groupedCarts) }})</strong> <strong
                                        class="fw-bold">USD
