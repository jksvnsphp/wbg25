...
...same header...
<h1>Deal Finalized</h1>

<p>Dear {{ $buyer_name }} & {{ $seller_name }},</p>

<p>Your deal for RFQ <strong>{{ $rfq_id }}</strong> is finalized.</p>

<ul>
<li><strong>Buyer:</strong> {{ $buyer_name }}</li>
<li><strong>Seller:</strong> {{ $seller_name }}</li>
<li><strong>Price:</strong> {{ $final_price }}</li>
</ul>

<p>Proceed with payment & shipping as agreed.</p>
<p><a href="{{ $dashboard_link }}" class="btn">Go to Dashboard</a></p>
