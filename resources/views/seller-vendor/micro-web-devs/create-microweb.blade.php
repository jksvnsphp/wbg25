                            ...
                            <div class="row d-flex align-items-center">
                                <div class="col-md-8">
                                    <div class="form-group mb-3">
                                        ...
                                        <div class="text-danger small mt-1" id="bannerError"></div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                ...
                <div class="card-body">
                    <div class="row">
                        @for ($i = 1; $i <= 4; $i++)
                            ...
                            <div class="form-group mb-3">
                                <label for="spotlight_image{{ $i }}" class="form-label fw-bold">Spotlight Product Picture:
                                    <span class="text-danger">(500px X 500px)</span></label>
                                <div class="row">
                                    <div class="col-md-8">
                                        <input type="file" name="spotlight_image{{ $i }}"
                                            class="form-control spotlight-input"
                                            id="spotlight_image{{ $i }}" accept="image/*" />
                                        <div class="text-danger small mt-1" id="featuredError{{ $i }}"></div>
                                    </div>
                                    <div class="col-md-4">
...
<script>
    function validateImage(file, expectedWidth, expectedHeight, maxSizeMB) {
        return new Promise((resolve, reject) => {
            ...
            if (file.size > maxSize) {
                reject(`Image size must be less than ${maxSizeMB}MB.`);
                return;
            }
            ...
            img.onload = function() {
                ...
                if (img.width !== expectedWidth || img.height !== expectedHeight) {
                    reject(`Image dimensions must be ${expectedWidth} × ${expectedHeight}px.`);
                } else {
                    resolve();
                }
            };
    ...
    // 🔹 Spotlight Banner Validation
    $('#profile_banner').on('change', function() {
        const file = this.files[0];
        const errorDiv = $('#bannerError');

        errorDiv.text('');

        if (!file) return;

        validateImage(file, 2520, 620, 2)
            .then(() => {
                // valid banner
            })
            .catch(error => {
    ...
    // 🔹 Featured Images Validation + Preview
    $('.spotlight-input').on('change', function() {
        const input = this;
        const file = input.files[0];
        const index = input.id.replace('spotlight_image', '');
        const previewImg = $('#previewImage' + index);
        const errorDiv = $('#featuredError' + index);

        errorDiv.text('');

        if (!file) return;

        validateImage(file, 500, 500, 2)
            .then(() => {
            ...
            .catch(error => {
                ...
                previewImg.attr(
                    'src',
                    '{{ asset("uploads/products/gallery/Image_not_available.png") }}'
                );
            });
    });
</script>
