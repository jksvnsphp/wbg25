                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 col-12 d-flex align-items-center">
                                        <div class="preview-wrapper position-relative">
                                            <img
                                                id="news_imagePreview"
                                                src="{{ asset('world-business/images/signin.jpg') }}"
                                                class="img-fluid rounded-2 d-none"
                                                style="height:6rem;width:6rem;border:1px solid #ccc;" />

                                            <button
                                                type="button"
                                                class="btn btn-danger btn-sm remove-image-btn d-none"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const MAX_SIZE_MB = 2;
        let newsImageValid = false;

        const input = document.getElementById('news_image');
        const preview = document.getElementById('news_imagePreview');
        ...
        input.addEventListener('change', function() {
            ...
            if (file.size > MAX_SIZE_MB * 1024 * 1024) {
                showError(`Image size must not exceed ${MAX_SIZE_MB}MB.`);
                reset();
                return;
            }
            ...
            img.onload = function() {
                ...
                if (img.width > maxWidth && img.height > maxHeight) {
                    showError(`Image too large. Max allowed size is ${maxWidth}×${maxHeight}px.`);
                    reset();
                    return;
                }
        ...
        function showError(message) {
            ...
            err.className = 'text-danger image-error d-block mt-1';
            err.innerText = message;
            input.closest('.form-group').appendChild(err);
        }

        function clearError() {
            const err = input.closest('.form-group').querySelector('.image-error');
            if (err) err.remove();
        }
