                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle">
                            ...
                            <tbody>
                                ...
                                <tr @if ($inbox_message->receiver_id === auth()->user()->id && (!$inbox_message->is_read || (($inbox_message->unread_chat_count ?? 0) > 0))) style="background-color: #f8d7da;" @endif>
                                    <td width="10%">
                                    ...
                                    <td>
                                        @php
                                        $latestMessageText = $inbox_message->latestChatMessage->message ?? $inbox_message->message;
                                        $latestMessageAt = $inbox_message->latestChatMessage->created_at ?? $inbox_message->created_at;
                                        @endphp
                                        <h6 class="fw-bolder fs-6">{{ Str::limit($inbox_message->subject, 30) }}
                                            @if (($inbox_message->unread_chat_count ?? 0) > 0)
                                            <span class="badge bg-danger ms-2">{{ $inbox_message->unread_chat_count }}</span>
                                            @endif
                                        </h6>
                                        {{ Str::limit($latestMessageText, 150) }}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        {{ \Illuminate\Support\Carbon::parse($latestMessageAt)->format('d F Y h:i A') }}
                                    </td>
