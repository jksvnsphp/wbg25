        ...
        <div class="row">
            <div class="col-md-12 mt-2 bg-primary py-3">
                <div class="d-flex align-items-center justify-content-start mb-3">
                    ...
                    <a href="{{ route('buyer.myquotations.show') }}"
                        class="btn btn-sm me-3 {{ Route::currentRouteName() == 'buyer.myquotations.show' ? 'btn-secondary disabled' : 'btn-light' }}">Active</a>
                    <a href="{{ route('buyer.myexpired.quotations.show') }}"
                        class="btn btn-sm {{ Route::currentRouteName() == 'buyer.myexpired.quotations.show' ? 'btn-secondary disabled' : 'btn-light' }}">Inactive</a>
                </div>
                                ...
                                <tbody>
                                    @foreach ($quotations as $quotation)
                                        <tr>
                                            ...
                                            <td>
                                                {{ Str::limit($quotation->requirement_details, 100) }}
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <a href="{{ route('buyer.edit.quotation',$quotation->id) }}" title="ReList" class="btn btn-sm btn-secondary">
                                                    <i class="fa fa-rotate"></i> Re-List
                                                </a>
                                            </td>
                                            <td>
                                                <div class="d-flex">
        ...
        $(document).ready(function() {
            $('.delete-quotation').on('click', function(e) {
                ...
                if (confirm("Are you sure you want to delete this quotation?")) {
                    $.ajax({
                        url: '{{ route('buyer.delete.quotation') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
