                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <h6 class="text-center">
                                                        Deal Done
                                                        {{ date('d M, Y', strtotime($quotationOffer->created_at)) }}</h6>
                                                    <h6 class="text-center">CET
                                                        {{ date('h:i A', strtotime($quotationOffer->created_at)) }}</h6>
                                                </td>
                                                <td>
                                                    <span class="text-secondary fw-semibold">
                                                        Quotation Deal Price: USD&dollar; {{ $quotationOffer->offer_price ?? 0 }}
                                                    </span>
                                                </td>
                                                <td>
                                        ...
                                        <div style="border-right: 1px solid #ddd"
                                            ...
                                            @if ($quotationOffer->shipment_status == 'delivered' || $quotationOffer->shipment_status == 'shipped')
                                                <p class="mt-3 d-block"><strong>Shipment Company:</strong>
                                                    {{ $quotationOffer->shippment_company ?? '' }}</p>
                                                <p><strong>Shipment Tracking Number:</strong>
                                                    {{ $quotationOffer->tracking_number ?? '' }}</p>
                                            @else
    ...
    @if ($quotationOffer->vendor_id == auth()->user()->id)
        <script>
            function changePayment(id, status) {
                ...
                let orderItemId = id;
                $.ajax({
                    url: '{{ route("buyer.status.deal-quotation") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
