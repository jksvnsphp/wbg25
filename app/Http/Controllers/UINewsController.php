...
{
    //
    public function index_old_05_06_(Request $request)
    {
        ...
        $newsQuery = SellerNews::where('isPublish', 1)->with('vendor.company')->latest();
        ...
        if ($request->has('subcategory') && $request->category != '') {
            $newsQuery->where('subcategory_id', $request->category);
        }
